...
def sigmoid(x: float, a: float, x0: float, k: float, b: float) -> float:
...
class ConfidenceScoreGenerator:
    ...
    @staticmethod
    def __acc(threshold: float, dist: np.array, issame: np.array) -> list:
    ...
    def __call__(self, cosine_distances, labels) -> list:
        ...
        with open("./debug.pkl", "wb") as f:
            pickle.dump([sigmoid_parameters, ratios], f)
        """
